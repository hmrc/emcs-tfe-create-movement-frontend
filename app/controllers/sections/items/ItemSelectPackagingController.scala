...
class ItemSelectPackagingController @Inject()(
        ...
        withGoodsTypeAsync(idx) { goodsType =>
          getPackagingTypesService.getItemPackagingTypes(Some(true)).flatMap { nonCountablePackagingTypes =>
            val selectItems = SelectItemHelper.constructSelectItems(
              ...
              existingAnswer = request.userAnswers.get(ItemSelectPackagingPage(idx)).map(_.code)
            )
            renderView(Ok, fillForm(ItemSelectPackagingPage(idx), formProvider(goodsType, nonCountablePackagingTypes)), idx, goodsType, selectItems, mode)
          }
        }
      }
    }
        ...
        withGoodsTypeAsync(idx) { goodsType =>
          getPackagingTypesService.getItemPackagingTypes(Some(true)).flatMap { nonCountablePackagingTypes =>
            ...
            formProvider(goodsType, nonCountablePackagingTypes).bindFromRequest().fold(
              renderView(BadRequest, _, idx, goodsType, selectItems, mode),
              saveAndRedirect(ItemSelectPackagingPage(idx), _, mode)
            )
          }
        }
      }
  ...
  private def renderView(status: Status, form: Form[_], idx: Index, goodsType: GoodsType, selectItems: Seq[SelectItem],mode: Mode)
                        (implicit request: DataRequest[_]): Future[Result] =
      Future.successful(status(view(
        form = form,
        action = routes.ItemSelectPackagingController.onSubmit(request.ern, request.draftId, idx, mode),
