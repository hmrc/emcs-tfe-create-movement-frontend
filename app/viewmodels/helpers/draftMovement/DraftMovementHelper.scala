  ...
  private[draftMovement] def submitSection(sectionsExceptSubmit: Seq[TaskListSection])
    ...
    TaskListSection(
      ...
      rows = Seq(TaskListSectionRow(
        ...
        id = "submit",
        link =  if (completed && (request.userAnswers.createdFromTemplateId == None)) Some(controllers.sections.templates.routes.SaveTemplateController.onPageLoad(request.ern, request.draftId).url) else if (completed) Some(controllers.routes.DeclarationController.onPageLoad(request.ern, request.draftId).url) else None,
        section = None,
