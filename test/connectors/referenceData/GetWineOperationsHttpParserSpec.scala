...
class GetWineOperationsHttpParserSpec extends SpecBase with GetWineOperationsHttpParser with MockHttpClient with ItemFixtures {

  override def http: HttpClient = mockHttpClient

  "GetWineOperationsReads" - {

    "should return a Seq[WineOperations]" - {

      s"when an OK (${Status.OK}) response is retrieved" in {

        val expectedResult = Right(testWineOperations)

        val jsonResponse = testWineOperationsJson

        val actualResult = new GetWineOperationsReads().read("", "", HttpResponse(Status.OK, jsonResponse.toString()))
    ...
    "should return UnexpectedDownstreamError" - {
      ...
      s"when status is anything else" in {
        ...
        val actualResult = new GetWineOperationsReads().read("", "", HttpResponse(Status.INTERNAL_SERVER_ERROR, ""))
