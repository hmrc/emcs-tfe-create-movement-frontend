...
class DestinationTypeControllerSpec extends SpecBase with MockUserAnswersService with MockPreDraftService {
  ...
  class Fixture(
    ...
    lazy val controller = new DestinationTypeController(
      ...
      fakeAuthAction,
      new FakePreDraftRetrievalAction(Some(userAnswersSoFar), Some(testMinTraderKnownFacts), Some(testMessageStatistics)),
      preDraftDataRequiredAction,
      new FakeDataRetrievalAction(Some(userAnswersSoFar), Some(testMinTraderKnownFacts), Some(testMessageStatistics)),
      dataRequiredAction,
      formProvider,
