...
import models.sections.items.ItemBulkPackagingCode.BulkGas
...
class ItemBulkPackagingSealTypeControllerSpec extends SpecBase with MockUserAnswersService with ItemFixtures {
  ...
  val action: Call = routes.ItemBulkPackagingSealTypeController.onSubmit(testErn, testDraftId, testIndex1, NormalMode)

  val baseUserAnswers: UserAnswers = emptyUserAnswers.set(ItemBulkPackagingSelectPage(testIndex1), BulkPackagingType(BulkGas, "Aerosol"))

  class Test(val optUserAnswers: Option[UserAnswers] = Some(emptyUserAnswers)) {
    lazy val request: FakeRequest[AnyContentAsEmpty.type] = FakeRequest()

    lazy val controller = new ItemBulkPackagingSealTypeController(
      messagesApi,
  ...
  "ItemBulkPackagingSealType Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in new Test(Some(
      baseUserAnswers.set(ItemBulkPackagingSealTypePage(testIndex1), ItemPackagingSealTypeModel("answer", None))
    )) {
      val result = controller.onPageLoad(testErn, testDraftId, testIndex1, NormalMode)(request)
    ...
    "must redirect to the next page when valid data is submitted" in new Test(Some(baseUserAnswers)) {
      ...
      val result = controller.onSubmit(testErn, testDraftId, testIndex1, NormalMode)(request.withFormUrlEncodedBody(("packaging-seal-type", "answer")))
    ...
    "must redirect to the next page when valid data is submitted and both fields are entered" in new Test(Some(baseUserAnswers)) {
      ...
      MockUserAnswersService.set(
        baseUserAnswers.set(ItemBulkPackagingSealTypePage(testIndex1), ItemPackagingSealTypeModel("answer", Some("other answer")))
      ).returns(Future.successful(
        baseUserAnswers.set(ItemBulkPackagingSealTypePage(testIndex1), ItemPackagingSealTypeModel("answer", Some("other answer")))
      ))
