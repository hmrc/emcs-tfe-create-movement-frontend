        ...
        "when there's an answer" - {
          ...
          "when the show action link boolean is true" - {
            ...
            "must output the expected row" in {
              ...
              implicit lazy val request = dataRequest(FakeRequest(), emptyUserAnswers.set(ConsigneeExportVatPage, "GB123456789"))
          ...
          "when the show action link boolean is false" - {
            ...
            "must output the expected row without action links" in {
              ...
              ConsigneeExportVatSummary.row(showActionLinks = false) mustBe
                Some(
                  SummaryListRowViewModel(
                    key = messagesForLanguage.cyaLabel,
                    value = Value(Text("GB123456789")),
