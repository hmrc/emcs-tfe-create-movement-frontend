...
class ItemNetMassSummarySpec extends SpecBase with Matchers {
    ...
    s"when being rendered in lang code of 'en'" - {
      ...
      "when there's no answer" - {
        ...
        "must output None" in {
          ...
          ItemNetMassSummary.row(testIndex1) mustBe None
        }
      }
      ...
      "when there's an answer" - {
        ...
        "must output the expected row" in {
          ...
          ItemNetMassSummary.row(testIndex1) mustBe Some(
            SummaryListRowViewModel(
              key = messagesForLanguage.cyaNetMassLabel,
              value = Value(Text(s"999.736 kg")),
              actions = Seq(
                ActionItemViewModel(
                  ...
                  id = "changeNetMass"
                ).withVisuallyHiddenText(messagesForLanguage.cyaNetMassChangeHidden)
              )
            )
          )
        }
